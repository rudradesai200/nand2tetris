...
enum tokentype {keywordConstant,keyword,symbol};
enum EntryType {Field,Static,Local,Argument,NONETYPE};
...
class class_symbol_table {
	...
	void addEntry(string varname,EntryType varkind,string vartype){
		int varind;
		if(varkind == Field){
			varind = Field_count++;
		}
	...
	public:
		...
		bool compileClassVarDec(){
			...
			if(p.first == "identifier"){
				outFile<<curr_line<<endl;
				varname = p.second;
				CT->addEntry(varname,varkind,vartype);
				p=extracttokens();
				while(checkSymbol(",",true,true,true)){
					p = extracttokens();
					if(p.first != "identifier"){(errFile)<<"ERROR: Expecting <identifier> but "<<p.second<<endl;return false;}
					varname = p.second;
					CT->addEntry(varname,varkind,vartype);
					outFile<<curr_line<<endl;
					p = extracttokens();
		...
		bool compileSubroutineDec(){
			...
			if(p.second == "constructor"){subkind = Constructor;}
			else if(p.second == "method"){subkind = Method;}
			else if(p.second == "function"){subkind = Function;}
			p = extracttokens();
		...
		bool compileParameterList(){
			...
			if((p.first == "keyword" && (p.second == "int" || p.second =="boolean" || p.second == "char" || p.second == classname)) || (p.first == "identifier")){
				...
				if(p.first == "identifier"){
					...
					ST->addEntry(varname,varkind,vartype);
					outFile<<curr_line<<endl;
					p = extracttokens();
					while(checkSymbol(",",true,true,true)){
						p = extracttokens();
		...
		bool compileVarDec(){
			EntryType varkind = Local;
				...
				else{(errFile)<<"ERROR: Expecting <keyword> but "<<p.second<<endl;return 0;}
				string vartype = p.second;
				p = extracttokens();
				string varname;
				if(p.first == "identifier"){varname = p.second;outFile<<curr_line<<endl;}
				else{ (errFile)<<"ERROR: Expecting <identifier> but "<<p.second<<endl;return 0;}
				ST->addEntry(varname,varkind,vartype);
		...
		bool compileStatements(){
			...
			while(p.first == "keyword"){
				...
				if(p.second == "let"){
					...
					outFile<<curr_line<<endl;
					if(!compileLetStatement()){return false;}
					outFile<<"</letStatement>"<<endl;
					p = extracttokens();
				}
				else if(p.second == "if"){
					...
					outFile<<curr_line<<endl;
					if(!compileIfStatement()){return false;}
					outFile<<"</ifStatement>"<<endl;
				}
				else if(p.second == "while"){
					outFile<<"<whileStatement>"<<endl;
					outFile<<curr_line<<endl;
					if(!compileWhileStatement()){return false;}
					outFile<<"</whileStatement>"<<endl;
					p = extracttokens();
				}
				else if(p.second == "do"){
					...
					outFile<<curr_line<<endl;
					if(!compileDoStatement()){return false;}
					outFile<<"</doStatement>"<<endl;
					p = extracttokens();
				}
					...
					outFile<<curr_line<<endl;
					if(!compileReturnStatement()){return false;}
					outFile<<"</returnStatement>"<<endl;
					p = extracttokens();
				}
				else{
		...
		bool compileOp(){
		  ...
		  if(p.first != "symbol"){
		    return false;
			  ...
			  if(p.second == "+"){outFile<<curr_line<<endl;opstack.push("add");}
			  else if(p.second == "-"){outFile<<curr_line<<endl;opstack.push("sub");}
			  else if(p.second == "&"){outFile<<curr_line<<endl;opstack.push("and");}
			  else if(p.second == "|"){outFile<<curr_line<<endl;opstack.push("or");}
			  else if(p.second == ">"){outFile<<curr_line<<endl;opstack.push("gt");}
			  else if(p.second == "<"){outFile<<curr_line<<endl;opstack.push("lt");}
			  else if(p.second == "="){outFile<<curr_line<<endl;opstack.push("eq");}
			  else if(p.second == "*"){outFile<<curr_line<<endl;opstack.push("call Math.multiply 2");}
			  else if(p.second == "/"){outFile<<curr_line<<endl;opstack.push("call Math.divide 2");}
			  else {
		...
		int compileTerm(){
			...
			if(checkSymbol("-",true,true,false)){
				...
				string id1 = p.second;
				int f= 1;
				symbol_table_entry var = ST->find(p.second);
					...
					if(p.second == "["){
						...
						if(p.first == "identifier"){
							...
							if(checkSymbol("(",false,false,true)){
								int nP = compileExpressionList();
								if((nP == -1)){return -1;}
								if(!checkSymbol(")",true,false,true)){return -1;}
								if(f == 1){
									vmFile<<"call "<<var.Type<<"."<<id2<<" "<<(nP + 1)<<endl;
								}
								else{
				...
				outFile<<curr_line<<endl;
				int check = compileExpression();
				if(check == 0){errFile<<"ERROR: Expression expected"<<endl;return -1;}
				if(check == -1){return -1;}
				if(checkSymbol(")",true,false,true)){
					p = extracttokens();
					outFile<<"</term>"<<endl;
		  ...
		  if(check > 0){
		    ...
		    while(compileOp()){
		      ...
		      if((check =compileTerm()) > 0){
		        ...
		        if(check == 0){errFile<<"ERROR: term expected"<<endl;outFile<<"</expression>"<<endl;};
				return false;
		      }
		    }
		...
		bool compileLetStatement(){
			...
			if(p.first == "identifier"){
				string varname = p.second;
				symbol_table_entry var = ST->find(varname);
				if(var.Index == -1){
					var = CT->find(varname);
					if(var.Index == -1){
						if(!(p.second == "Array" || p.second == "Output" || p.second == "Keyboard" || p.second == "Screen" || p.second == "Memory" || p.second == "Sys"))
				...
				if(checkSymbol("[",true,true)){
					...
					if((check = compileExpression()) <= 0){
						if(check == -1){return false;}
						else{errFile<<"ERROR: Expression expected"<<endl;return false;}
					}
					if(checkSymbol("]",true)){
						outFile<<curr_line<<endl;
						vmFile<<"push "<<kindmap[var.Kind]<<" "<<var.Index<<endl;
		    ...
		    label1:
				if(checkSymbol("=",true)){
					...
					if((check = compileExpression()) > 0){
						...
						else{vmFile<<"pop "<<kindmap[var.Kind]<<" "<<var.Index<<endl;}
						if(checkSymbol(";",true)){
							outFile<<curr_line<<endl;
						}
						else{
				...
				if(p.first == "symbol" && p.second == "."){
					...
					p = extracttokens();
						if(p.first == "identifier"){
							...
							outFile<<curr_line<<endl;
							int f= 1;
							symbol_table_entry var = ST->find(id1);
							if(var.Index == -1){
								var = CT->find(id1);
								if(var.Index == -1){
									f = 2;
							...
							if(checkSymbol("(",false,false,true)){
								...
								if(!checkSymbol(")",true,false,true)){return 0;}
								if(!checkSymbol(";",false,false,true)){return 0;}
								if(f == 1){
									vmFile<<"call "<<var.Type<<"."<<id2<<" "<<(nP + 1)<<endl;
									vmFile<<"pop temp 0"<<endl;
								}
